...
총판매금액이 가장 높은 제품은 리피다운정 20mg이고, 가장 높은 영업 부서는 호남영업팀이다. 
총판매금액을 기준으로 실적을 비교해본 결과 제품은 상위 그룹이 하위 그룹에 비해 약 10배 이상 높았고,
영업팀은 약 3.47배 높은 수준을 보였다.
또한 거래 건수가 적더라도 건당 판매금액이 높은 경우 총 판매금액이 크게 나타는 것도 확인할 수 있었다.
...
where sale_date between '2025-04-01' and '2025-06-30'
	and sales_type != '반품'
group by sd.sales_dept_id, sd.sales_dept_name, sd.sales_region 
having sum(sale_amount) >= 30000000
order by total_sales_amount desc;
